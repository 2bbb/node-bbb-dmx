...
class ControllerUnit {
    ...
    constructor(name, blueprint, addresses, default_buffer, opt = {}) {
        ...
        if(osc_port) {
            ...
            osc_port.forEach(port => {
                functional.range(1, this.length + 1).forEach(i => {
                    const osc_address = `/${this.name}/${i}`;
                    this.registeredOsc.push(osc_address);
                    osc.subscribe(port, osc_address, ([address, ...values]) => {
                        functional.object.forEach(this.controllers, controller => {
                            controller.set(i, values[0]);
                        });
                    });
                });
                pairs.forEach(pair => {
                    ...
                    if(util.isFunction(pair.dmx)) {
                        ...
                        if(pair.osc.search(/\/set$/) != -1) {
                            osc.subscribe(port, pair.osc.replace(/\/set$/, ""), ([address, ...values]) => {
                                functional.object.forEach(this.controllers, (controller) => {
                                    pair.dmx(values, controller.buffer, controller.address - 2, controller);
                                });
                            });
    ...
    get power() { return functional.object.reduce(this.controllers, (reduced, value) => reduced + value.power, 0); }
    ...
    get dip() {
        return functional.object.mapKV(this.controllers, controller => ({key: controller.name, value: controller.dip}));
    }
    ...
    set(n, value) {
        if(Pack.isPack(value)) {
            return value.apply((v, key) => this.controllers[key].set(n, v[key]));
        } else {
            return this.map((controller, key) => controller.set(n, value));
        }
    }
    get(n) {
        return Pack.apply(this.controllers, controller => controller.get(n));
    }
    extend() {
        for(const key in this.addresses) {
        ...
        Object.keys(blueprint).forEach(key => {
            ...
            if(util.isFunction(value)) {
                Object.defineProperty(object, key, {
                    get: () => (...v) => {
                        return this.map(controller => value(v, controller.buffer, controller.address - 2, controller));
                    },
                    set: (v) => {
                        return this.map(controller => value(v, controller.buffer, controller.address - 2, controller));
                    }
                ...
                if(value.hasOwnProperty('set')) {
                    Object.defineProperty(object, key, {
                        ...
                        set: (v) => {
                            if(Pack.isPack(v)) {
                                return this.map((controller, k) => value.set(v[k],
                                                                             controller.buffer,
                                                                             controller.address - 2,
                                                                             controller));
                            } else {
                                return this.map(controller => value.set(v, controller.buffer, controller.address - 2, controller));
                            }
                        }
                    });
                } else {
    ...
    map(callback) {
        return Pack.apply(this.controllers, callback);
    }
}

ControllerUnit.Pack = Pack;
